...
description:
  ...
  or "/common:jira-update". Enforces status transition discipline, comment standards (one per day,
  delivery-focused), and issue linking conventions. This is the canonical way to keep Jira accurate as the
...
Use `mcp__atlassian__getTransitionsForJiraIssue` to retrieve valid transitions from the current status, then
`mcp__atlassian__transitionJiraIssue` to apply the change.
...
Always use `getTransitionsForJiraIssue` to confirm which transitions are valid from the ticket's current status —
...
### Step 3 — Link issues (if needed)

- Use the branch naming convention (`<type>/<ticket-id>-<desc>`) to auto-link PRs/MRs where your git host supports
  it
- Use `blocks` / `is blocked by` for dependencies between tickets
- Use `relates to` for loosely connected items
- Never rely on text references alone — use Jira's linking feature via `mcp__atlassian__createIssueLink`

## Edge cases and failure modes

- **Ticket not found** — surface the MCP error; never update the wrong ticket.
- **Transition not available** — call `getTransitionsForJiraIssue` to list what is valid from the current status;
...
- **Already commented today** — if a comment was added earlier today, update or append to it rather than creating
  a second one.
- **Blocked ticket** — when adding a blocker, always create the issue link in addition to mentioning it in the
  comment, so it appears in the Jira UI.
